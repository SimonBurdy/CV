...
<head>
    ...
    <title>DEVIS n°{{ $quote->number }}</title>
    ...
    <div>
                ...
                <td class="address">
                    <div >
                        ...
                        @if($quote->clientAddress)
                            <strong> {{$quote->clientAddress->name}} </strong>
                            <p>{!! nl2br(e($quote->clientAddress->address))!!} </p>
                        @else
                            <strong>{{ $quote->project->client->name }} </strong>
        ...
        <table style="width:100%;">
            <tr rowspan="2">
                <th rowspan="2">
                        <table style="width:100%; border-collapse:" cellpadding=0 cellspacing=10>
                            ...
                            <tr >
                                <td colspan="2" >
                                    <span>Date de devis</span>
                                </td>

                                <td>
                                    <span>{{ \Carbon\Carbon::parse($quote->creation_date)->format('d/m/Y')}}</span>
                                </td>
                            </tr>
            ...
            <tr>
                <td >
                    <div>
                        <strong>Projet n° {{ $quote->project->project_number }}</strong>
                        <p> {{ $quote->notes }}</p>
                    </div>
                </td>
            </tr>
        ...
        <table class="page_break_footer table_article" cellpadding=0 cellspacing=0>
            ...
            @foreach($quote->rows as $row)
                ...
                @endif
            @endforeach
            @if($quote->discount_euro > 0)
                <tr class="tr_tbody_article" style="border-top: 1px solid rgb(190, 187, 187);">
                    <td class="line" style="text-align: left;">
                        <strong>Geste commercial</strong>
                        <p>Remise de {{  $quote->discount_euro }} pour Devis n° {{ $quote->number }} d'un montant
                            de {{ $quote->sell_total }} € HT</p>
                    </td>
                    ...
                    <td class="line">
                        - {{  number_format($quote->discount_euro, 2 , ',',' ')}} €
                    </td>
                    <td style="border-bottom: 1px solid rgb(190, 187, 187); text-align: center">
                        - {{  number_format($quote->discount_euro, 2 , ',',' ')}} €
                    </td>
                        ...
                        <table cellpadding=0 cellspacing=0 style="width: 100%">
                            <tr>
                                ...
                                <td style="text-align: right; padding-right : 3px;">
                                    <p>{{ number_format($quote->sell_total, 2 , ',',' ') }} €</p>
                                    
                                </td>
                            </tr>
                            <tr>
                                <td >
                                    <p>TVA {{ $quote->vat }}%  {{ number_format($quote->sell_total, 2 , ',',' ') }} €</p>
                                </td>
                                <td style="text-align: right ; padding-right : 3px;">    
                                    <p>{{ number_format($quote->total_vat, 2 , ',',' ') }} €</p>
                                </td>
                            ...
                            <tr class="result_total">
                                ...
                                <td style="text-align: right ; padding-top: 3px ; padding-bottom: 3px; padding-left: 3px;" >
                                    <strong> {{ number_format($quote->total_with_taxes, 2 , ',',' ') }} € </strong>
                                </td>
                            </tr>
